#!/usr/bin/env python3
"""
Script para crear usuario administrador y registrar todos los productos.
"""
...
def crear_usuario_admin():
    ...
    # Crear usuario administrador usando bcrypt directamente
    contrasena_hash = bcrypt.hashpw(ADMIN_PASSWORD.encode('utf-8'), bcrypt.gensalt()).decode('utf-8')
    try:
        cursor.execute("""
            INSERT INTO usuarios (nombre, apellido_paterno, correo, contrasena, rol, activo)
            VALUES (%s, %s, %s, %s, %s, %s)
        """, ("Admin", "Sistema", ADMIN_EMAIL, contrasena_hash, "superadministrador", True))
        conexion.commit()
...
def main():
    ...
    print("2️⃣ Verificando/creando usuario administrador...")
    if not crear_usuario_admin():
        print("❌ No se pudo crear/verificar el usuario administrador")
        sys.exit(1)
    print("")
